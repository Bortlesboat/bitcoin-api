...
class _McpBrowserRedirect:
    """Raw ASGI middleware: redirect GET /mcp → /mcp-setup for browsers.

    Must be raw ASGI because Starlette's Mount swallows /mcp at the router
    level before HTTP middleware runs.
    """
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "http" and scope["path"] == "/mcp" and scope["method"] == "GET":
            ...
            response = _RR(url="/mcp-setup", status_code=302)
...
def register_middleware(app: FastAPI):
    ...
    # Redirect GET /mcp → /mcp-setup (must be ASGI-level, before Mount routing)
    app.add_middleware(_McpBrowserRedirect)
